...
public class CustomLoginSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    ...
    // 로그인 성공 직후 수행할 동작.
    // 이 사람이 언제 로그인 했고 얼마나 우리 홈피에 머물러 있었는가 매우 중요!
    @Override
    public void onAuthenticationSuccess(HttpServletRequest request,
        ...
        // 로그인 시간을 세션에 저장하기 (나중에 logout 할 때 사용할 거다)
        LocalDateTime loginTime = LocalDateTime.now();
        System.out.println("로그인 시간 : " + loginTime);
        request.getSession().setAttribute("loginTime", loginTime); // 로그인시간 setAtt.로 저장

        // 로그인 직전의 url 로 redirect 해주기
