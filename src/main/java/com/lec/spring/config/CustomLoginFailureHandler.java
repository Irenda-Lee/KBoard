...
/*
 * 실패한 Exception 을 검사하여 해당 Exception 에 맞는 에러 메시지를 로그인 페이지에 같이 전달하여
 * 로그인 실패 이유를 사용자에게 노출시킵니다.
 *
 * 아래에 구현된 Exception 을 포함한 AuthenticationException 의 종류는 다음과 같습니다.
...
public class CustomLoginFailureHandler implements AuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        ...
        //< expired the credential
                ...
                // 다 했으면 redirect(다시 재요청) 혹은 forward(서버 내부에서 새로운 리퀘함) 시켜주기
