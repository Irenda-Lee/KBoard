...
public class CustomLogoutSuccessHandler implements LogoutSuccessHandler {
    ...
    public void onLogoutSuccess(HttpServletRequest request
       ...
       request.getSession().invalidate(); // 세션 invalidate. 세션에 저장돼 들어있던거 끄집어내기
       String redirectUrl = "/user/login?logoutHandler";

       // 만약 ret_url(리턴하고자 하는 url) 이 있는 경우 logout 하고 해당 url 로 리다렉!
      if(request.getParameter("ret_url") != null) {
          redirectUrl = request.getParameter("ret_url");
      };

       response.sendRedirect(redirectUrl); // 이거 한 다음에는 뭔가 처리하지 말 것!!
    }
}
