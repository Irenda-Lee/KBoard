...
public class PrincipalOauth2UserService extends DefaultOAuth2UserService {
    ...
    // 인증 직후 호출되는 loadUser()는 구글로부터 받은 userRequest 데이터에 대한 후처리를 진행함
    @Override
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        //   → code 리턴 (Oauth2 client 라이브러리가 받음) → AccessToken 요청
        ...
        //  ㄴ dev.yml 에서 설정한 registration 값 넘어옴
        ...
        if (user == null) { // 미가집자인 경우에만 회원가입 진행
            ...
            if (cnt > 0) {
                ...
                user = userRepository.findByUsername(username);  // 다시 DB 에서 읽어와야 한다. id, regDate 등의 정보
            } else {
        ...
        return principalDetails; // 결국 OAuth2User 를 리턴한 셈~!
    }
}
